...
void TriangulationWidget::mouseReleaseEvent(QMouseEvent* e)
{
    if (!polygonSaved)
    {
                ...
                if (fabs(node->getCoordX() - e->pos().x()) < 10 && fabs(node->getCoordY() - e->pos().y()) < 10)
                {
                    ...
                    else
                    {
                        ...
                        if (isPolygonClosed())
                        {
                            ...
                            Triangulation::triangulate(polygon, diagonals);
                            if (!diagonals.empty()) {
...
void TriangulationWidget::paintEvent(QPaintEvent* e)
{
...
void TriangulationWidget::mouseMoveEvent(QMouseEvent* e)
{
    if (!polygonSaved)
    {
...
void TriangulationWidget::mousePressEvent(QMouseEvent* e)
{
    if (!polygonSaved)
    {
        std::vector<Node*> nodes = polygon.getNodes();
...
bool TriangulationWidget::isPolygonClosed()
{
    std::vector<Node*> nodes = polygon.getNodes();
    std::vector<Edge*> edges = polygon.getEdges();
    int counter = 0;
